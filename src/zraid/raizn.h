...
// #define IGNORE_ORDERING // udelay() to keep write ordering ignore
// #define HIGH_QD_SUPPORT // write ordering is less strict, allowed in zrwa area
...
typedef enum {
	RAIZN_STRIPE_WAIT_WR, // data, PP, or full parity are submitted. wait for completion
	RAIZN_STRIPE_WAIT_ZF, // ZRWA explicit flush was submitted
	RAIZN_STRIPE_COMPLETE	
} raizn_stripe_state;
...
struct raizn_sub_io *raizn_stripe_head_alloc_bio(struct raizn_stripe_head *sh, struct bio_set *bioset, int bvecs, sub_io_type_t sub_io_type, void *data);
struct raizn_stripe_head *raizn_stripe_head_alloc(struct raizn_ctx *ctx, struct bio *bio, raizn_op_t op);
